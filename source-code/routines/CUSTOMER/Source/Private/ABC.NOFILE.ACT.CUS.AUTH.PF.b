...
*---------------------------------------------------------------
    ...
    Y.NO.REG= ''
    Y.REG.LIST = ''
*    Y.SEL.CMD = "SELECT " : FN.CUSTOMER$NAU : " WITH SECTOR EQ 1001 AND CUSTOMER.TYPE EQ ACTIVE"
    Y.SEL.CMD = "SELECT " : FN.CUSTOMER$NAU : " WITH CUSTOMER.TYPE EQ ACTIVE"
    Y.REG.LIST = '' ; Y.NO.REG = ''
    EB.DataAccess.Readlist(Y.SEL.CMD, Y.REG.LIST, '', Y.NO.REG, Y.ERROR)
    GOSUB IMPRIME.TODO
...
*---------------------------------------------------------------
    ...
    WHILE Y.I LE Y.NO.REG
        ...
        R.DATA<-1> = Y.REG.ACT:Y.SEP:Y.SHORT.NAME:Y.SEP:Y.NAME.ONE:Y.SEP:Y.NAME.TWO:Y.SEP:Y.SECTOR:Y.SEP:'Persona Fisica':Y.SEP:Y.ID.OFFICER:Y.SEP:Y.DAO.NAME:Y.SEP:'Por Activar':Y.SEP:Y.INPUTTER:Y.SEP:Y.COMPANY
