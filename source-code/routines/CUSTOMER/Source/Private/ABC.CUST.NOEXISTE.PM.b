...
*---------------------------------------------------------------
	...
	END ELSE
		Y.CLASSIFICATION.POS = ""
	    EB.LocalReferences.GetLocRef("CUSTOMER","CLASSIFICATION",Y.CLASSIFICATION.POS)
	    Y.CLASSIFICATION = R.CUSTOMER<ST.Customer.Customer.EbCusLocalRef, Y.CLASSIFICATION.POS>
	    		    
	    IF Y.CLASSIFICATION EQ 1 THEN
	    	Y.EXISTE.ERROR = 1
	    	Y.MENSAJE = "UTILICE LA OPCION DE MANTENIMIENTO PARA PERSONA FISICA PARA EL CLIENTE " : Y.NUM.CUSTOMER
	    END
	    
	    IF Y.CLASSIFICATION EQ 2 THEN
	    	Y.EXISTE.ERROR = 1
