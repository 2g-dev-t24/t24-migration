...
*===============================================================================
    ...
    EB.SystemTables.setRNew((EB.CUS.TAX.ID)<1,1>,"")       ;* RFC
    EB.SystemTables.setRNew((EB.CUS.EXTERN.CUS.ID)<1,1>,"")         ;* CURP
...
*.. INICIALIZA VARIABLES
    ...
    IF ( EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.CLASSIFICATION> = "1") OR ( EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.CLASSIFICATION> = "2") THEN
        GOSUB VALIDA.DATOS
        IF (RES = 0) AND (MENSAJE = "") THEN
            ...
            EB.SystemTables.setRNew((EB.CUS.EXTERN.CUS.ID)<1,1>,CLIENTE.UNICO.CURP)
    ...
    IF EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.CLASSIFICATION> = "3" THEN
        EB.SystemTables.setRNew(EB.CUS.GENDER,'')
        GOSUB VALIDA.DATOS.M
        IF (RES = 0) AND (MENSAJE = '') THEN
            GOSUB CALCULA.NUM.CTE.M

            EB.SystemTables.setRNew((EB.CUS.TAX.ID)<1,1>,CLIENTE.UNICO)
            EB.SystemTables.setRNew((EB.CUS.EXTERN.CUS.ID)<1,1>,"")
            EB.Display.RebuildScreen()
            RETURN
        END ELSE
            ETEXT = MENSAJE
...
CALCULA.NUM.CTE:
    ...
    APE.PATERNO  = EB.SystemTables.getRNew(EB.CUS.SHORT.NAME)
    APE.MATERNO  = EB.SystemTables.getRNew(EB.CUS.NAME.1)

******************************************************************************************************************
* Elimino los campos locales para el nombre 2 y el nombre 3
    Y.CUS.NAME.2 = EB.SystemTables.getRNew(EB.CUS.NAME.2)
...
******************************************************************************************************************
    ...
    IF Y.APE.PAT > 1 THEN; GOSUB VALIDA.APE; END
    Y.APE.MAT = DCOUNT(APE.MATERNO, ' ')
    IF Y.APE.MAT > 1 THEN; GOSUB VALIDA.APE.2; END
    ...
    IF (Y.APELLIDO.1[1,3] = 'XXX') OR (Y.APELLIDO.1 = 'XX') OR (Y.APELLIDO.1 = 'X') OR (Y.APELLIDO.1 = 'X ') OR (Y.APELLIDO.1 = 'XX ') THEN
        Y.APELLIDO.1 = 0
    END
    IF (Y.APELLIDO.2[1,3] = 'XXX') OR (Y.APELLIDO.2 = 'XX') OR (Y.APELLIDO.2 = 'X') OR (Y.APELLIDO.2 = 'X ') OR (Y.APELLIDO.2 = 'XX ') THEN
        Y.APELLIDO.2 = 0
    END

    IF (Y.APELLIDO.1 = 0) THEN
        CLIENTE.UNICO.RFC[1,2] = Y.APELLIDO.2[1,2]; CLIENTE.UNICO.CURP[1,2] = Y.APELLIDO.2[1,2]; BAND = 1
    END ELSE
        IF (Y.APELLIDO.2 = 0) THEN
...
*..MUEVE PRIMER LETRA DE PRIMER APELLIDO
            IF NOT((LONG.APE.1 = 1) OR (LONG.APE.1 = 2)) THEN
                CLIENTE.UNICO.RFC[1,1] = Y.APELLIDO.1[1,1]; CLIENTE.UNICO.CURP[1,1] = Y.APELLIDO.1[1,1]
*..BUSCA PRIMERA VOCAL DEL PRIMER APELLIDO
                V1 = 1
                LOOP
                    V1 = V1 + 1
                UNTIL (((Y.APELLIDO.1[V1,1] = 'A') OR (Y.APELLIDO.1[V1,1] = "E") OR (Y.APELLIDO.1[V1,1] = "I") OR (Y.APELLIDO.1[V1,1] = "O") OR (Y.APELLIDO.1[V1,1] = "U")) OR (V1 > LEN(Y.APELLIDO.1)))
                REPEAT

                IF (V1 <= LEN(Y.APELLIDO.1) ) THEN
...
*..VALIDANDO QUE LAS PRIMERAS 4 POSICIONES
    ...
    FINDSTR CLIENTE.UNICO.RFC[1,4] IN LIST.C SETTING POS.RFC THEN
        IF POS.RFC NE '' THEN
            ...
            IF CLIENTE.UNICO.RFC[1,4] = POS.RFC[1,4] THEN
...
*..MUEVA FECHA DE NACIMIENTO
    CLIENTE.UNICO.RFC[5,6]  = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)[3,6]
    CLIENTE.UNICO.CURP[5,6] = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)[3,6]
*..MUEVE GENERO
...
*..MUEVE PRIMERA CONSONANTE DE PRIMER APELLIDO
    ...
    LOOP
        V1 = V1 + 1
    UNTIL (((Y.APELLIDO.1[V1,1] <> 'A') AND (Y.APELLIDO.1[V1,1] <> 'E') AND (Y.APELLIDO.1[V1,1] <> 'I') AND (Y.APELLIDO.1[V1,1] <> 'O') AND (Y.APELLIDO.1[V1,1] <> 'U')) OR (V1 > LEN(Y.APELLIDO.1)))
    REPEAT
...
*..MUEVE PRIMERA CONSONANTE DE SEGUNDO APELLIDO
    ...
    LOOP
        V1 = V1 + 1
    UNTIL (((Y.APELLIDO.2[V1,1] <> 'A') AND (Y.APELLIDO.2[V1,1] <> "E") AND (Y.APELLIDO.2[V1,1] <> "I") AND (Y.APELLIDO.2[V1,1] <> "O") AND (Y.APELLIDO.2[V1,1] <> "U")) OR (V1 > LEN(Y.APELLIDO.2)))
    REPEAT
...
*..MUEVE PRIMERA CONSONANTE DE PRIMER NOMBRE
    ...
    LOOP
        V1 = V1 + 1
    UNTIL ((Y.NOMBRE[V1,1] <> 'A') AND (Y.NOMBRE[V1,1] <> "E") AND (Y.NOMBRE[V1,1] <> "I") AND (Y.NOMBRE[V1,1] <> "O") AND (Y.NOMBRE[V1,1] <> "U")) OR (V1 > LEN(Y.NOMBRE))
    REPEAT
    CLIENTE.UNICO.CURP[16,1] = Y.NOMBRE[V1,1]

    FOR CON.POS = 1 TO 16
        Y.MAYUS = CHAR(165)
        IF (CLIENTE.UNICO.CURP[CON.POS,1] = Y.MAYUS) OR (CLIENTE.UNICO.CURP[CON.POS,1] = ' ') OR (CLIENTE.UNICO.CURP[CON.POS,1] = '') THEN
            CLIENTE.UNICO.CURP[CON.POS,1] = 'X'
        END
    NEXT CON.POS

    FOR CON.POS = 1 TO 10
        IF (CLIENTE.UNICO.RFC[CON.POS,1] = Y.MAYUS) OR (CLIENTE.UNICO.RFC[CON.POS,1] = ' ') THEN
...
CALCULA.NUM.CTE.M:
    ...
    NOMBRE.EMP = EB.SystemTables.getRNew(EB.CUS.NAME.1)
    ...
    END ELSE
        CLIENTE.UNICO[1,3] = EB.SystemTables.getRNew(EB.CUS.NAME.1)[1,3]
    END

    CLIENTE.UNICO[4,6] = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)[3,6]
...
NOMBRE.INVALIDO:
    ...
    IF ((PRIMER.NOM = "MARIA") OR (PRIMER.NOM = "MA")OR (PRIMER.NOM = "MA.") OR (PRIMER.NOM = "JOSE" )) THEN
    ...
    Y.NOMBRE = NOM.COMPLETO
    PRIMER.NOM = FIELD(Y.NOMBRE, ' ', 1)
    IF ((PRIMER.NOM = "MARIA") OR (PRIMER.NOM = "MA")OR (PRIMER.NOM = "MA.") OR (PRIMER.NOM = "JOSE" ) OR (PRIMER.NOM = "J" ) OR (PRIMER.NOM = "J." )) THEN
        ELIM.NOM = LEN(PRIMER.NOM) + 1  ;**... ELIMINO EL PRIMER NOMBRE Y EL ESPACIO EN BLANCO. EJEMPLO: MARIA*DE*LOURDES, QUEDA ASI: DE*LOURDES
        Y.NOMBRE[1, ELIM.NOM] = ''
        Y.NOM = DCOUNT(Y.NOMBRE, ' ')
        IF Y.NOM > 1 THEN
...
VALIDA.APE:
    ...
    FOR Y.PAT = 1 TO Y.APE.PAT
        ...
        FINDSTR A.APE IN LIST.D SETTING POS.APE THEN
            ...
            END
        END
        IF (POS.APE = '') AND (POS.APE NE ' ') THEN
            APE.PATERNO = ''
            APE.PATERNO = A.APE
            BREAK
        END
...
VALIDA.APE.2:
    ...
    FOR Y.MAT = 1 TO Y.APE.MAT
        ...
        FINDSTR B.APE IN LIST.D SETTING POS.APE.2 THEN
            IF NOT(B.APE = LIST.D<POS.APE.2>) THEN
                POS.APE.2 = ''
            END
        END
        IF (POS.APE.2 = '') AND (POS.APE.2 NE ' ') THEN
            APE.MATERNO = ''
            APE.MATERNO = B.APE
            BREAK
        END
...
VALIDA.NOM:
    ...
    FOR Y.FOR.NOM = 1 TO Y.NOM
        ...
        FINDSTR C.NOM1 IN LIST.D SETTING POS.NOM THEN
            IF NOT(C.NOM1 = LIST.D<POS.NOM>) THEN
                POS.NOM = ''
            END
        END
        IF (POS.NOM = '') AND (POS.NOM NE ' ') THEN
            Y.NOMBRE = ''
            Y.NOMBRE = C.NOM1
            BREAK
        END
...
VALIDA.EMPRESA:
    ...
    FOR Y.EMPRESA = 1 TO Y.VAL.EMP
        ...
        FINDSTR D.EMP IN LIST.D SETTING POS.EMP THEN
            IF NOT(D.EMP = LIST.D<POS.EMP>) THEN
                POS.EMP = ''
            END
        END
        IF POS.EMP = '' THEN
            VAL.TMP = D.EMP
        END
        IF (POS.EMP = '') AND (POS.EMP NE ' ') AND (LEN(RFC.EMP < 3)) THEN
            IF Y.EMPRESA = Y.VAL.EMP THEN
                RFC.EMP := VAL.TMP[1,2]
            END ELSE
                RFC.EMP := VAL.TMP[1,1]
            END
        END
        IF (Y.EMPRESA = Y.VAL.EMP) AND (LEN(RFC.EMP < 3)) THEN
            RFC.EMP := VAL.TMP[2,1]
        END
    NEXT
RETURN
...
**.. VALIDA LOS APELLIDOS Y NOMBRE INGRESADOS PARA EVITAR ERRORRES EN EL SELECT. EJEMPLO: APE.PAT: DE LEON, APE.MAT: DE LA GARZA, NOMBRE1: MARIA DE LOS, NOMBRE2: ANGELES
    ...
    Y.APE.PAT = EB.SystemTables.getRNew(EB.CUS.SHORT.NAME)
    Y.APE.MAT = EB.SystemTables.getRNew(EB.CUS.NAME.1)
    Y.NOMBRE  = EB.SystemTables.getRNew(EB.CUS.NAME.2)

    NO.APE.PAT = DCOUNT(EB.SystemTables.getRNew(EB.CUS.SHORT.NAME), ' ')
    IF NO.APE.PAT > 1 THEN
        Y.APE.PAT = FIELD(EB.SystemTables.getRNew(EB.CUS.SHORT.NAME), ' ', NO.APE.PAT)
        Y.APE.PAT = '...':Y.APE.PAT     ;**... SE TOMARIA "...LEON"
    END

    NO.APE.MAT = DCOUNT(EB.SystemTables.getRNew(EB.CUS.NAME.1), ' ')
    IF NO.APE.MAT > 1 THEN
        Y.APE.MAT = FIELD(EB.SystemTables.getRNew(EB.CUS.NAME.1), ' ', NO.APE.MAT)
        Y.APE.MAT = '...':Y.APE.MAT     ;**... SE TOMARIA ASI "...GARZA"
    END


    NOMBRE.COMP = EB.SystemTables.getRNew(EB.CUS.NAME.2)
    ...
    NOMBRE.NUEVO := " " : EB.SystemTables.getRNew(EB.CUS.SHORT.NAME):" "
    NOMBRE.NUEVO := EB.SystemTables.getRNew(EB.CUS.NAME.1)

    NO.NOMBRE = DCOUNT(NOMBRE.COMP, ' ')

    IF NO.NOMBRE > 1 THEN
        Y.NOMBRE = FIELD(EB.SystemTables.getRNew(EB.CUS.NAME.2)<1,1>, ' ', 1)
...
**.. SE HACE UNA BUSQUEDA CON EL APE.PAT, APE.MAT, NOMBRE1, FEC.NAC, LUG.NAC Y SEXO INGRESADO, PARA VER SI YA FUE CAPTURADO ANTERIORMENTE
    ...
    Y.ID.CLIENTE = ID.NEW
    Y.RFC = EB.SystemTables.getRNew(EB.CUS.TAX.ID)<1,1>
    Y.DATE.BIRTH = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)
    Y.CURP = EB.SystemTables.getRNew(EB.CUS.EXTERN.CUS.ID)

    IF EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.CLASSIFICATION> GE "3" THEN
        Y.RFC.AUX = Y.RFC[1,9]
        Y.ESTADO = EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,YPOS.LUGAR.CONST>
        Y.NOM.PER.MORAL = TRIM(EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,YPOS.NOM.PER.MORAL>)
        GOSUB VALIDA.CLIENTE.PM
    END ELSE
        Y.RFC.AUX = Y.RFC[1,10]
        Y.ESTADO = EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.LUG.NAC>
...
*Campos Locales
    ...
    V.APP      = 'CUSTOMER'
    V.FLD.NAME = 'CLASSIFICATION' : @VM : 'LUG.NAC': @VM : 'NOM.PER.MORAL': @VM : 'LUGAR.CONST'
    V.FLD.POS  = ''

    EB.Updates.MultiGetLocRef(V.APP, V.FLD.NAME, V.FLD.POS)
    
    Y.POS.CLASSIFICATION = V.FLD.POS<1,1>
    Y.POS.LUG.NAC        = V.FLD.POS<1,2>
    YPOS.NOM.PER.MORAL   = V.FLD.POS<1,3>
    YPOS.LUGAR.CONST     = V.FLD.POS<1,4>
...
********************
    APE.PATERNO = EB.SystemTables.getRNew(EB.CUS.SHORT.NAME)
    APE.MATERNO = EB.SystemTables.getRNew(EB.CUS.NAME.1)
    NOMBRE      = EB.SystemTables.getRNew(EB.CUS.NAME.2)
    FEC.NAC     = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)
    LUG.NAC     = EB.SystemTables.getRNew(EB.CUS.LOCAL.REF)<1,Y.POS.LUG.NAC>
    GENERO      = EB.SystemTables.getRNew(EB.CUS.GENDER)
...
VALIDA.DATOS.M:
    NOMBRE.EMP = EB.SystemTables.getRNew(EB.CUS.NAME.1)
    FEC.CONSTI = EB.SystemTables.getRNew(EB.CUS.DATE.OF.BIRTH)
    ...
    IF LEN(FEC.CONSTI) = 0 THEN
        ...
        EB.SystemTables.setRNew(EB.CUS.DATE.OF.BIRTH,Y.COMI)
    END
RETURN
