$PACKAGE ABC.BP
SUBROUTINE ABC.NOFILE.ACT.CUS.AUTH.PFAE(R.DATA)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
PROCESA:
*---------------------------------------------------------------
    Y.SEL.CMD = "SELECT " : FN.CUSTOMER : " WITH SECTOR EQ 1100 AND CUSTOMER.TYPE EQ PROSPECT"
    Y.REG.LIST = '' ; Y.NO.REG = ''
    ...
    Y.NO.REG= ''
    Y.REG.LIST = ''
    Y.SEL.CMD = "SELECT " : FN.CUSTOMER$NAU : " WITH SECTOR LIKE ...1100... AND CUSTOMER.TYPE EQ ACTIVE"
    Y.REG.LIST = '' ; Y.NO.REG = ''
    EB.DataAccess.Readlist(Y.SEL.CMD, Y.REG.LIST, '', Y.NO.REG, Y.ERROR)
...
*---------------------------------------------------------------
    ...
    WHILE Y.I LE Y.NO.REG
        ...
        R.DATA<-1> = Y.REG.ACT:Y.SEP:Y.SHORT.NAME:Y.SEP:Y.NAME.ONE:Y.SEP:Y.NAME.TWO:Y.SEP:Y.SECTOR:Y.SEP:'Persona Fisica Act Empre':Y.SEP:Y.ID.OFFICER:Y.SEP:Y.DAO.NAME:Y.SEP:Y.ESTADO:Y.SEP:Y.INPUTTER:Y.SEP:Y.COMPANY
