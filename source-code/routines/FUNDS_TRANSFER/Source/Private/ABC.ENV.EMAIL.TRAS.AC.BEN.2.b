* @ValidationCode : MjotODg3MjIyNzExOkNwMTI1MjoxNzQ0NzU2NTkzMzM4OlVzaWFyaW86LTE6LTE6MDowOmZhbHNlOk4vQTpSMjRfU1AxLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Apr 2025 17:36:33
...
*===============================================================================
* Nombre de Programa : ABC.ENV.EMAIL.TRAS.AC.BEN.2
...
*    CALL GET.LOC.REF('FUNDS.TRANSFER','EXT.TRANS.ID', POS.EXT.TRANS.ID)
    ...
    Y.NOMBRE.CAMPO<2,1> = "EXT.TRANS.ID"
    R.POS.CAMPO = ''
    ...
    POS.EXT.TRANS.ID = R.POS.CAMPO<2,1>
    ...
    Y.NO.CUENTA = EB.SystemTables.getRNew(FT.Contract.FundsTransfer.CreditAcctNo) ;*R.NEW(FT.CREDIT.ACCT.NO)
    Y.NO.CUENTA.ORD = EB.SystemTables.getRNew(FT.Contract.FundsTransfer.DebitAcctNo) ;*R.NEW(FT.DEBIT.ACCT.NO)
    ...
    Y.EXT.TRANS.ID = sYLocalRef<1, POS.EXT.TRANS.ID> ;*R.NEW(FT.LOCAL.REF)<1,POS.EXT.TRANS.ID>
...
*-----------------------------------------------------------------------------
    ...
    Y.ID.CUS = R.CUENTA<AC.AccountOpening.Account.Customer> ;*<AC.CUSTOMER>
    Y.WORKING.BALANCE = R.CUENTA<AC.AccountOpening.Account.WorkingBalance> ;*<AC.WORKING.BALANCE>
    sYLocalRef = EB.SystemTables.getRNew(FT.Contract.FundsTransfer.LocalRef)
    Y.CANAL.ENTIDAD = sYLocalRef<1, POS.CANAL.ENTIDAD> ;*R.CUENTA<AC.LOCAL.REF, POS.CANAL.ENTIDAD>
    Y.PRN = sYLocalRef<1, Y.POS.PRN.ACC> ;*R.CUENTA<AC.LOCAL.REF, Y.POS.PRN.ACC>
...
*    REC.ABC.SMS.EMAIL.ENVIAR<ABC.EMA.CUENTA.CLIENTE>   =   Y.NO.CUENTA.ORD
    REC.ABC.SMS.EMAIL.ENVIAR<ABC.BP.AbcSmsEmailEnviar.AbcSmsEmailEnviarCuentaCliente>   =   Y.NO.CUENTA.ORD
