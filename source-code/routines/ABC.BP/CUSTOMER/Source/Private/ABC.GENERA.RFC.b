...
PROCESO:
    ...
    EB.Updates.MultiGetLocRef('CUSTOMER','CLASSIFICATION',CTE.CAMPOS.LOCALES)

    POS.CLASSIFICATION = FIELD(CTE.CAMPOS.LOCALES,'#',1)

    IF IN.ID.CLIENTE NE "" THEN

        CTE.CLASSIFICATION = CTE.RS<ST.Customer.Customer.EbCusLocalRef, POS.CLASSIFICATION >
    END ELSE
        Y.LOCAL.REF = EB.SystemTables.getRNew(ST.Customer.Customer.EbCusLocalRef)
        CTE.CLASSIFICATION = Y.LOCAL.REF<1, POS.CLASSIFICATION >
    END
...
SET.MOR.REGLA.10:
    ...
    FOR  A.I.PALABRAS = 1 TO A.CANT.PALABRAS
        ...
        IF ISDIGIT(A.PALABRA) THEN
            ...
            CALL VPM.CANTIDAD.LETRA(A.PALABRA, A.PALABRA, '', '', Y.ERR.LETRA)
